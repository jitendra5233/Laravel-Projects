...
class Property extends Controller
{
    ...
    public function GetPropertySearchFilter(Request $req){
        ...
        if($req->minPrice != 00){
            $newArr = [];
            foreach($result as $row){
                if($req->minPrice <= str_replace(",","",$row->price)){
                    $newArr[] = $row;
                }
            }
            $result = $newArr;
        ...
        if($req->maxPrice != 00){
            $newArr = [];
            foreach($result as $row){
                if($req->maxPrice >= str_replace(",","",$row->price)){
                    $newArr[] = $row;
                }
            }
            $result = $newArr;
    ...
    public function GetPropertySingle(Request $req){
        ...
        foreach($result as $row){
                ...
                $pcount = PropertyStatusModel::where('id',$row->propertyListingStatus)->get();
